...
namespace Com.Trakindo.TSICS.Business.Service
{
    public class PISBusinessServices
    {
        ...
        public List<CalcPIS> InsertDataCalcPIS()
        {
            ...
            var listItem = new List<CalcPIS>();
            foreach (var item in getDataRebuild)
            {
                var list = new CalcPIS();
                ...
                list.Area = item.location.Area;
                list.Region = item.location.Region;
                list.SalesOffice = item.location.SalesOffice;
